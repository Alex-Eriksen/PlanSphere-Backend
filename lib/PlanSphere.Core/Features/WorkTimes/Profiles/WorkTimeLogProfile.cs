...
public class WorkTimeLogProfile : Profile
{
    public WorkTimeLogProfile()
    {
        CreateMap<WorkTimeRequest, WorkTimeLog>()
            ...
            .ForMember(dest => dest.ActionType, opt => opt.MapFrom((src, dest, destMember, context) => MappingHelpers.GetEnumFromContext<ActionType>(context, "ActionType") ?? ActionType.Create))
            .ForMember(dest => dest.OldStartDateTime, opt => opt.MapFrom((src, dest, destMember, context) => 
            {
                var workTime = MappingHelpers.GetFromContext<WorkTime>(context, "WorkTime");
